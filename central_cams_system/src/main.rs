...
impl CamsSystem {
    ...
    fn delete_camara(&mut self, id: u8) -> Result<Option<Cam>, Error> {
        ...
        if cam.state == CamState::Alert {
            return Err(Error::new(
                std::io::ErrorKind::Other,
                "No se puede eliminar una cámara en modo alerta",
            ));
        }
    ...
    pub fn list_cameras(&self) {
        for cam in self.system.cams.iter() {
            println!(
                "Cámara: id:{} - modo:{:?} - latitud:{} - longitud:{}",
...
fn process_messages(
    ...
    camsystem: Arc<Mutex<CamsSystem>>,
            ...
            match incident.state {
                IncidentState::InProgess => {
                    ...
                    match client.publish(
                        serialize_cams_vec(camsystem.system.cams.clone()),
                        "camaras".to_string(),
                    ) {
                        Ok(_) => {}
                        Err(e) => {
                ...
                IncidentState::Resolved => {
                    let mut camsystem = camsystem.lock().unwrap();
                    camsystem
                        .modify_cameras_state(incident.location.clone(), CamState::SavingEnergy);
                    match client.publish(
                        serialize_cams_vec(camsystem.system.cams.clone()),
                        "camaras".to_string(),
                    ) {
            ...
            Ok(line) => {
                ...
                match *action {
                    "add" => {
                        ...
                        match client.publish(
                            serialize_cams_vec(cam_system.system.cams.clone()),
                            "camaras".to_string(),
                        ) {
                            Ok(_) => {}
                            Err(e) => {
...
fn show_menu_options() {
    println!(
        "Bienvenido al sistema de cámaras, a continuación se muestran las opciones disponibles."
    );
    println!("Opciones:");
...
fn main() -> Result<(), Error> {
    ...
    client.publish(
        serialize_cams_vec(cam_system_clone.lock().unwrap().system.cams.clone()),
        "camaras".to_string(),
    )?;
