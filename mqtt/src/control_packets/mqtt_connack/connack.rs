...
use crate::control_packets::mqtt_connect::{connect::Connect, variable_header_properties::VariableHeaderProperties};

use super::{connect_reason_code::ConnectReasonMode, fixed_header::ConnackFixedHeader, variable_header::ConnackVariableHeader};
...
impl Connack {
    pub fn write_to(&self, stream: &mut dyn Write) -> Result<(), Error> {
        ...
        let variable_header_connect_acknowledge_flags_length = self.variable_header.connect_acknowledge_flags.to_be_bytes();
        let variable_header_connect_reason_code_length = self.variable_header.connect_reason_code.to_be_bytes();
    ...
    pub fn read_from(stream: &mut dyn Read) -> Result<Self, Error> {
        ...
        let variable_header_acknowledge_flags = u8::from_be_bytes(read_variable_header_acknowledge_flags);
...
fn determinate_reason_code(_connect_packet: Connect) -> u8 {
    let connect_reason_code = ConnectReasonMode::Success.get_id();
    connect_reason_code
}

fn create_connect_acknowledge_flags(
